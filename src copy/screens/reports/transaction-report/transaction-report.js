...
const TransactionReportScreen = ({navigation}) => {
  ...
  const [fromDatePickerVisibility, setFromDatePickerVisibility] =
    useState(false);
  ...
  const [datecodeResult, setDatecodeResult] = useState(allFilteredDateCodeResults ? allFilteredDateCodeResults : null); // set default
  ...
  const onOpenSuggestionsList = useCallback(isOpened => {}, []);
    ...
    //if(search){
      if(search.length > 3)
      {
        dispatch(fetchTranactionBySearchTextAction(search.toUpperCase()));
       
        const resultFiltered = allFilteredResults.map(item => ({
      ...
      setSuggestionsList(resultFiltered);
      setLoading(false);
      } else {
