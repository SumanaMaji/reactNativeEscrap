...
const TransactionImage = props => {
  ...
  const [cdnLargeImage, setCdnLargeImage] = useState(null);
  ...
  useEffect(() => {
   ...
   // setLargeImageData(transactionImages.image);
  }, []);
  ...
  const removeImageHandler = imageId => {
    ...
    //   readData('user_token');
    // }
    //console.log(Urls.deleteTransactionThumbnail);
  ...
  function showCdnThumbnailImage(item) {
    return (
      <Image
  ...
  function showCdnLargeImage(imageUrl) {
    return (
      <Image
        ...
        renderItem={({item}) => {
          return (
            <View>
              <TouchableOpacity
                key={item.imageId}
                onPress={() => showLargeImage(item.thumbnailUrl)}>
                {
                  showCdnThumbnailImage(item)
                }
              </TouchableOpacity>
              <Text style={styles.cardimageTitle}>{item.thumbnailName}</Text>
            </View>
      ...
      <Modal
        ...
        <View style={styles.centeredView}>
          <View style={styles.modalView}>
            ...
            {showCdnLargeImage(cdnLargeImage)}
            <TouchableOpacity
