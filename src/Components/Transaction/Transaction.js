...
const Transaction = props => {
  ...
  const handleSubmitButton = async () => {
    ...
    if (vehicleRC == '') {
      alert('Please choose Vehicle RC');
      return;
    }
    if (challan == '') {
      alert('Please choose challan');
      return;
    }
    if (photo.length < 10) {
      alert('Minimum 10 images is required to create a transaction.');
      return;
    }
    //Show Loader
    setLoading(true);
    ...
    let dataToSend = {
      ...
      gatePassDate: Moment(gatePassDate).format('YYYY-MM-DD'),
      remark: remarks,
      images: images,
      photo: driverPhoto,
